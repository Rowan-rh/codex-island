...
struct ProviderConnectionTests {
    ...
    static func main() throws {
        ...
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.selected == [.grok, .antigravity, .codex], "repairs invalid and duplicate preferences")
        defaults.set(["grok", "antigravity", "codex", "claude", "deepseek"], forKey: ProviderVisibilityStore.selectionKey)
        store = ProviderVisibilityStore(defaults: defaults)
        expect(store.selected == [.grok, .antigravity, .codex, .claude], "caps selection at four providers")
        store.set(.claude, at: 0)
        expect(store.selected == [.claude, .antigravity, .codex, .grok], "choosing an extra provider for a slot swaps it in")
        store.toggleExtra(.antigravity)
        expect(store.selected.count == 4, "slot providers are not toggled as extras")
        store.set(nil, at: 1)
        expect(store.selected == [.claude], "single-provider choice also drops the extras")
        store.set(.codex, at: 1)
        store.toggleExtra(.grok)
        store.toggleExtra(.grok)
        expect(store.selected == [.claude, .codex], "extras can be removed")
        store.toggleExtra(.jev)
        store.toggleExtra(.grok)
        store.toggleExtra(.deepseek)
        expect(store.selected == [.claude, .codex, .jev, .grok], "extras stop at four providers")
